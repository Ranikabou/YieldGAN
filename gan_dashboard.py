        ...
        <body class="bg-gray-100">
            ...
            <div class="max-w-7xl mx-auto px-4 py-8">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                    ...
                    <!-- Training Progress Bar -->
                    <div id="training-progress-section" class="mt-4 p-4 bg-green-50 rounded-lg border border-green-200" style="display: none;">
                        <h4 class="text-md font-medium text-green-700 mb-2">📊 Overall Training Progress</h4>
                        <div class="w-full bg-gray-200 rounded-full h-3 mb-2">
                            <div id="overall-progress-bar" class="bg-green-600 h-3 rounded-full transition-all duration-300" style="width: 0%"></div>
                        </div>
                        <div class="flex justify-between text-sm text-green-600">
                            <span>Epoch <span id="overall-epoch">0</span> of <span id="total-epochs">10</span></span>
            ...
            <script>
                ...
                function updateProgress(data) {
                    ...
                    if (data.type === 'progress') {
                        ...
                        if (!progressBar) {
                            ...
                            if (trainingControls && trainingControls.parentNode) {
                                ...
                                if (mainContainer) {
                                    ...
                                    console.warn('Could not find container to insert progress container');
                                }
                            }
                        ...
                        // Update training progress display in status cards
                        const trainingProgressElement = document.getElementById('training-progress-display');
                        const trainingEpochElement = document.getElementById('training-epoch-display');
                        if (trainingProgressElement) {
                        ...
                        const overallProgressBar = document.getElementById('overall-progress-bar');
                        const overallProgressText = document.getElementById('overall-progress-text');
                        const overallEpoch = document.getElementById('overall-epoch');
                        
                        if (overallProgressSection && overallProgressBar && overallProgressText && overallEpoch) {
                            // Show the progress section when we get progress data
                            overallProgressSection.style.display = 'block';
                            
                            // Update the overall progress bar
                            overallProgressBar.style.width = `${progressPercent}%`;
                            overallProgressText.textContent = `${progressPercent.toFixed(1)}% complete`;
                            overallEpoch.textContent = data.epoch || 0;
                            
                            // Calculate overall training progress across all epochs
                            const totalEpochsElement = document.getElementById('total-epochs');
                            if (totalEpochsElement) {
                                const totalEpochs = parseInt(totalEpochsElement.textContent) || 10;
                                const currentEpoch = parseInt(data.epoch) || 0;
                                ...
                                const overallProgress = ((completedEpochs + (progressPercent / 100)) / totalEpochs) * 100;
                                
                                // Update overall progress display
                                const overallProgressText = document.getElementById('overall-progress-text');
                                if (overallProgressText) {
                                    overallProgressText.textContent = `Overall: ${overallProgress.toFixed(1)}% complete`;
                                }
                            }
                        }
